...
今天它实现了两种资源——**文件系统**与 feature 门控的 **Redis KV**——并附带一个让问题变得鲜活的例子：多个 AI coding agent 共享一个仓库。
...
2. **write**：在租约内变更——Limen 校验目标落在区域内、施加变更、记一条见证（字节数、内容哈希、时间、agent），并把它哈希链接到前一条见证
...
就这样——Claude Code、Cursor、Codex 等任何 MCP host 现在都能调用四个 tool（参数以文件系统资源为例）：
...
limen verify --db .limen/state.db         # 端到端校验见证哈希链
```

每条见证都被哈希链接（对其事实——包括归因的 agent——连同前一条见证的哈希做 SHA-256），任何打开 state db 的命令都会重走整条链并**失败即拒绝**：就地篡改、删除见证、改写 agent 标签都会被检出，而不是被当作事实渲染。
...
| **见证（witness）** | 一次中介变更的留痕证据 | 目标、字节数、SHA-256、时间、agent——哈希链接到前一条见证，每次打开时校验 |
...
| 🚫 **现在不是** | 强制中介（它是建议式的——发放并见证，不拦截）；跨机；超出 opt-in acquire 签名的身份（lease ID 仍是 bearer token） |
...
| 资源 | 两种（文件系统；`--features redis` 之后的 Redis KV）；模型对资源可插拔 |
| 强制 | **仅建议式**——agent 可绕过；见证仍归因 |
| 见证完整性 | **哈希链**（`prev_hash` → 对含归因 agent 在内的见证事实做 SHA-256），链头固定；每次打开与 `limen verify` 都失败即拒绝地校验。对就地篡改与删除可防；对拿到 db 文件写权限后的整链重算**不可防**（那需要把链头锚定在外部） |
