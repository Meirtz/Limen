...
| **身份（identity）** | 谁在请求 | 明文 agent 标签（`claude-code:sess-A`）；计划 ed25519 |
...
指标：**pass@1**（以及更严的 **pass^k**，度量重复并发下的可靠性）、**wall-clock**、**lost-edit-lines**、**build-break-rate**、**归因准确率**。完整可执行设计见 [`docs/experiments.md`](docs/experiments.md)；related work 与框架见 [`docs/spec/related-work.md`](docs/spec/related-work.md)。*（这是评估计划，不是已测结果。）*
