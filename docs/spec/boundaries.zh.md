...
**通用范畴（持久的定义）。** Limen 协调**在共享可变状态上并发操作的自治 agent**，凭三个原语：

- 一张建议式、按边界划定、带时限的**租约（lease）**（对命名空间某区域的写入授权），
- 一条**见证（witness）**轨迹（对每次中介写入的归因与取证），
- 以及一个 per-agent 的**身份（identity）**。

边界——*limen*，拉丁语"门槛"——是命名空间中的一块区域。范畴的根在分布式系统的并发控制与零信任安全，不在 LLM 编排。

**滩头（今天的首个实例——一个 case，不是定义）。** 多个 AI coding harness（Claude Code、Cursor、Codex、Gemini CLI）及其并行 sub-agent 在单机上共用一棵 git 工作树，以 MCP server 接入，建议式优先。这里共享状态是文件系统，区域是路径模式——但原语并不绑定于文件或 coding。
...
滩头刻意窄。下表每一维都展示*同一*原语如何泛化——并表明设计中没有任何东西被焊死在代码文件上。
...
策略是**持有范畴、同时只发一个窄的首个实例**——正是 Git 从内核源码、MCP 从桌面端走过的弧线。纪律是：让范畴指引 Limen *可能*成为什么，但不让它膨胀 Limen *今天是*什么。
